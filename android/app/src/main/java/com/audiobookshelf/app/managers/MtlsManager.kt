 ...
 * BLOCKING and MUST be called from a background thread, never from the main thread - use
 * [scope] (or one of the *Async helpers) rather than GlobalScope so this work is cancellable
 * and doesn't outlive the process in a way lint would flag.
 */
object MtlsManager {
  ...
  /** Shared background scope for mTLS work that call sites would otherwise run on GlobalScope. */
   ...
   *
   * BLOCKING — must be called from a background thread. Prefer [refreshForServerAsync] from
   * a call site that isn't already on one.
   */
  fun refreshForServer(serverConfigId: String) {
    val alias = getAliasForServer(serverConfigId)
    val sslContextAndTrustManager =
            alias?.let { getKeyAndChain(it) }?.let { (privateKey, certChain) ->
              buildSslContext(alias, privateKey, certChain)
            }

    if (sslContextAndTrustManager != null) {
      val (sslContext, trustManager) = sslContextAndTrustManager
      // Cache for getClient()
  ...
  /** Same as [refreshForServer], off the calling thread; invokes [onComplete] afterward. */
  fun refreshForServerAsync(serverConfigId: String, onComplete: () -> Unit = {}) {
    scope.launch {
      refreshForServer(serverConfigId)
      onComplete()
    }
  }
  ...
  fun getAliasForServer(serverConfigId: String): String? {
    // initialize() runs from MainActivity.onCreate() in the real app; guarded here so a caller
    // reached before that (or, as it turns out, a plugin test that never touches mTLS at all)
    // degrades to "no certificate configured" instead of an UninitializedPropertyAccessException.
  ...
  fun buildOkHttpClientForAlias(
    ...
    val (sslContext, trustManager) = buildSslContext(alias, privateKey, certChain) ?: return null
    val builder = OkHttpClient.Builder().sslSocketFactory(sslContext.socketFactory, trustManager)
    if (connectTimeout > 0) builder.connectTimeout(connectTimeout, TimeUnit.SECONDS)
    if (callTimeout > 0) builder.callTimeout(callTimeout, TimeUnit.SECONDS)
   ...
   *
   * BLOCKING — must be called from a background thread. Prefer [applyAsGlobalDefaultForAliasAsync]
   * from a call site that isn't already on one.
   */
  fun applyAsGlobalDefaultForAlias(alias: String) {
    val (privateKey, certChain) = getKeyAndChain(alias) ?: return
    val (sslContext, _) = buildSslContext(alias, privateKey, certChain) ?: return
    if (originalDefaultSocketFactory == null) {
      originalDefaultSocketFactory = HttpsURLConnection.getDefaultSSLSocketFactory()
    }
    HttpsURLConnection.setDefaultSSLSocketFactory(sslContext.socketFactory)
  ...
  /** Same as [applyAsGlobalDefaultForAlias], off the calling thread. */
  fun applyAsGlobalDefaultForAliasAsync(alias: String, onComplete: () -> Unit = {}) {
    scope.launch {
      applyAsGlobalDefaultForAlias(alias)
      onComplete()
    }
  }
  ...
  /**
   * Builds the SSLContext/TrustManager pair shared by every mTLS entry point above. Returns null
   * (and logs) on any failure building the context, so every caller can treat that uniformly as
   * "fall back to plain TLS" rather than each catching its own exception.
   */
  private fun buildSslContext(
    ...
    return try {
      ...
      Pair(sslContext, trustManager)
    } catch (e: Exception) {
      Log.e(tag, "buildSslContext: failed to build mTLS context for alias=$alias", e)
      null
    }
  }
